...
void IOCALL OPNIF::Reset(uint, uint)
{
	...
	try
	{
		if (m_rpcClient)
		{
			...
			for (int i = 0; i < 8; i++)
			{
				m_rpcClient->async_call("DirectAccessToChip", (unsigned char)23, (unsigned char)deviceNo,
					(unsigned int)0x28, (unsigned int)i);
			}
			for (int i = 0; i < 0xe; i++)
			{
				m_rpcClient->async_call("DirectAccessToChip", (unsigned char)23, (unsigned char)deviceNo,
					(unsigned int)(0x40 + i), (unsigned int)127);
				m_rpcClient->async_call("DirectAccessToChip", (unsigned char)23, (unsigned char)deviceNo,
					(unsigned int)(0x140 + i), (unsigned int)127);
			}
			//SSG
			m_rpcClient->async_call("DirectAccessToChip", (unsigned char)23, (unsigned char)deviceNo,
				(unsigned int)(0x7), (unsigned int)0x3f);
			//ADPCM
			m_rpcClient->async_call("DirectAccessToChip", (unsigned char)23, (unsigned char)deviceNo,
				(unsigned int)(0x101), (unsigned int)0x01);   //RESET
			m_rpcClient->async_call("DirectAccessToChip", (unsigned char)23, (unsigned char)deviceNo,
				(unsigned int)(0x100), (unsigned int)0x00);   //STOP
		}
	}
	catch (...)
	{
...
//	LOG2("Write0[%.2x] = %.2x\n", a, data);
	if (enable)
	{
		...
		try
		{
			if (m_rpcClient && index0 != 0x20)
				m_rpcClient->async_call("DirectAccessToChip", (unsigned char)23, (unsigned char)deviceNo,
					(unsigned int)index0, (unsigned int)data);
		}
		catch (...)
		{
		}
...
//	LOG2("Write1[%.2x] = %.2x\n", a, data);
	...
	if (enable && opnamode)
	{
		...
		try
		{
			if (m_rpcClient)
				m_rpcClient->async_call("DirectAccessToChip", (unsigned char)23, (unsigned char)deviceNo,
					(unsigned int)(0x100 | index1), (unsigned int)data);
		}
		catch (...)
		{
		}
